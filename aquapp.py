...
realtime_window = CTkToplevel()
realtime_window.geometry("1920x1080+3840+0")
realtime_window.attributes("-fullscreen", True)
...
title_label = CTkLabel(master=title_frame, text="Membrane Distillation", font=("Helvetica", 44, 'bold'))
...
for i, section in enumerate(sections):
    ...
    section_label = CTkLabel(master=section_frame, text=section, font=("Helvetica", 24, 'bold'), fg_color="#ffede0", corner_radius=15)
    ...
    if section == "Anaerobic CSTR":
        for j, (param, topic, col, unit) in enumerate(parameters[i]):
            ...
            value_label = CTkLabel(master=param_frame, text=f"{value}{unit}", font=("Helvetica", 32, 'bold'))
            ...
            param_label = CTkLabel(master=param_frame, text=f"{param}", font=("Helvetica", 20, 'bold'))
...
copyright_label = CTkLabel(master=footer_frame, text="All rights reserved © 2024 Pentaprism Technologies.", font=("Helvetica", 12))
